...
def arr_median(arr):
	...
	for col in range(cols):
		...
		ret.append(int(median))
	return ret

def arr_percentile(arr, percentile):
	ret = []
	cols = len(arr[0])
	rows = len(arr)
	for col in range(cols):
...
try:
	...
	print "Usage: " + sys.argv[0] + " house_id date period percentile"
	print "Example: " + sys.argv[0] + " OWC43DC7B0AE78 2012-06-01 30 0.5"
	sys.exit(0)
...
# Note: 0.5 percentile is median
percentile_all_traffic = []
percentile_shiftable_traffic = []
if abs(percentile - 0.5) < 0.001:
	percentile_all_traffic = arr_median(all_usage)
	percentile_shiftable_traffic = arr_median(shiftable_usage)
else:
	percentile_all_traffic = arr_percentile(all_usage, percentile)
	percentile_shiftable_traffic = arr_percentile(shiftable_usage, percentile)
...
ax.plot(date, percentile_all_traffic, 'o-', label='All Traffic')
ax.plot(date, percentile_shiftable_traffic, 'o-', label='Shiftable Traffic')
ax.xaxis.set_major_formatter(timeformat)
fig.autofmt_xdate()
plt.legend(loc='best')

percentile = int(percentile * 100);
filename = houseid + "_all_vs_shiftable_usage_" + str(percentile) + "_percentile.png"
